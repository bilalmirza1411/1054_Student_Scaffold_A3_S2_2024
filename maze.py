...
class Maze:
    ...
    def quick_escape(self, stamina: int) -> List[Position] | None:
        """
        FIT1054 STUDENTS ONLY
        Finds a way out of the maze if there is one within the 
        constrained stamina. The path must be the shortest path out of the maze.
        You must return the path taken in the maze from the start to the exit.

        Args:
            stamina(int): The stamina you have to escape the maze.

        Returns:
           List[Position] | None: If you have enough stamina to reach the exit
           path will be made up of the coordinates starting at
           your original starting point and ending at the exit. 
